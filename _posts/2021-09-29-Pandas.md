---
title: 2021-09-29-Pandas-Blog-Post
layout: post
author: skimball_22
post-image: "![elena-loshina-94c2BwxqwXw-unsplash](https://user-images.githubusercontent.com/77635875/133832882-606db727-44cd-4202-9b30-5a44f69019be.jpg)"
description: This will give a quick tutorial of how to get set up in python with pandas and give two intermediate functionalities.
tags:
- Beginner Pandas
- Multi Indexing
- tag3
---

**Why was Pandas Created?**

Pandas was created in order to get "better performance" than excel around 2007 by Wes McKinney. 

If you would like to learn more about the history of pandas, refer to this website
https://en.wikipedia.org/wiki/Pandas_(software)

**How to Install Pandas?**
1. Open up a new terminal window
<img width="566" alt="Screen Shot 2021-09-22 at 11 24 43 AM" src="https://user-images.githubusercontent.com/77635875/134392310-694ce656-3fea-4312-9180-e8a6fffeae7f.png">
2. Type in and run "pip install pandas"
<img width="569" alt="Screen Shot 2021-09-22 at 11 25 15 AM" src="https://user-images.githubusercontent.com/77635875/134392328-0c4a8299-d93b-4d89-bb11-2b9a15f83d17.png">

If you want to install pandas through the anaconda distribution, refer to this website
https://docs.anaconda.com/anaconda/navigator/tutorials/pandas/

**How to Use Pandas?**
1. Now you can import pandas in your text editor of choice in python
2. Import pandas in the first code chunk and save it as the common alias of pd
<img width="910" alt="Screen Shot 2021-09-22 at 11 41 13 AM" src="https://user-images.githubusercontent.com/77635875/134394415-d04f7eae-51d4-4459-9785-b55b8ed9ba17.png">
If you want to follow along, you can use your own dataset or if you would like to use the same tennis dataset, you can download it here:
https://www.kaggle.com/gmadevs/atp-matches-dataset/version/2?select=atp_matches_2017.csv

Don't forget to have the dataset saved in the same directory as where your python file is located at.
<img width="495" alt="Screen Shot 2021-09-22 at 11 49 25 AM" src="https://user-images.githubusercontent.com/77635875/134395542-862d4040-d32e-4bc1-a601-170755fa1578.png">

3. The type of the df object is now a pandas DataFrame
<img width="952" alt="Screen Shot 2021-09-22 at 11 43 44 AM" src="https://user-images.githubusercontent.com/77635875/134394640-987a3d71-9d4e-45a5-999b-1665988a7df9.png">
4. At this point, you no longer need to explicitly reference the alias pd since we have already loaded the data using pandas. The data is now a pandas object which means we can now use the different features of pandas.


**What are the benefits of using Pandas?**

**Issues with Pandas**

**Ways to Resolve Issues**

**Multi-Level Indexing**
Basically a glorified way to access elements after a group by occurs. Except it is not actually a group by that makes a pandas data frame have multiple index levels. You choose a column that you want to set the index to (so the column has to have more of a unique value per row), and then you are able to slice your data frame based on two different indeces. 

Refer to data camp's explanation
https://www.datacamp.com/community/tutorials/pandas-multi-index

** Date and Time **

** One or two functionalities that we haven't covered in class **


